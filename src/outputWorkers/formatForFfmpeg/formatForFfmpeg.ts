...
function getTargetMiddleOffset({
  source,
  target,
}: Omit<StreamDescription, 'z'>): FfmpegPositionCoords {
...
function formatLabelMargins(
  opt: Pick<VoxTilerTitleInput, MarginPaddings | 'margin' | 'padding'>
): Required<Pick<VoxTilerTitleInput, MarginPaddings>> {
  const readyMargins = {
...
function generateLabelWithRectangle(params: {
  baseRect: FfmpegPositionCoords;
  labelSettings: VoxTilerTitleInput;
}): {
  rectangle: RectangleDescription;
  ...
  const position: ['top' | 'bottom' | 'middle', 'left' | 'center' | 'right'] =
    labelSettings.position.split(' ') as ['top' | 'bottom' | 'middle', 'left' | 'center' | 'right'];
  let targetLabelX = 0;
  let targetLabelY = 0;
  switch (position[0]) {
    case 'top':
  ...
  input.forEach((record) => {
    if (record.stream) {
      ...
      if (record.stream.title) {
        const labelInfo = generateLabelWithRectangle({
          baseRect: blackRectangle,
          labelSettings: record.stream.title,
        });
        rectangles.push({
          ...labelInfo.rectangle,
          z: zIndexCounter.next().value || 0,
