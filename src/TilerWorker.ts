...
import {calculateSocketLinearSize, fixSocketAspectRatio, generateMargins, HALF, makeId} from "./utils";
...
class TilerWorker {
  ...
  private hashAreas(): AreaHash {
    return this.settings.areas.reduce<AreaHash>((acc, area, index): AreaHash => {
      ...
      if (area.overflow && area.overflow === 'next' && typeof this.settings.areas[index + 1] !== 'undefined') {
  ...
  private getGrid(area:number,count:number):VoxTilerGridDefinition{
    ...
    const resultGrid =  areaSets.area.grid.find(grid=>grid.fromCount<=count && (!grid.toCount||grid.toCount>=count));
    if(!resultGrid) {
  ...
  private separateInputByAreas(input: VoxTilerInput[]): Record<number, VoxTilerInput[]> {
    return input.reduce<Record<number, VoxTilerInput[]>>((acc, item): Record<number, VoxTilerInput[]> => {
      ...
      const checkOverflowRecurrent = (area: number): number => {
        ...
        if (typeof overflow !== 'undefined' && this.areaHash[overflow]) {
          this.log(`[${item.id}]`, 'Area', area, 'is full but can be overflow to area ', this.areaHash[area].overflow);
  ...
  private generateSockets(areaId:number,streamsCount:number):SocketPosition[]{
    ...
    const [width,height,lRoundPixel,tRoundPixel] = fixSocketAspectRatio(rawWidth,rawHeight,grid,area,margins);
    this.log(`Socket data: area [${area.width},${area.height}] socket [${width},${height}] round:[${lRoundPixel},${tRoundPixel}]`);
